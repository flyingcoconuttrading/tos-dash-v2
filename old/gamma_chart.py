# plotly imported lazily inside chart classes
...
def calculate_walls(data: dict, strikes: list, option_symbols: list) -> tuple:
    """
    Call Wall  — strike with highest call OI. Acts as resistance.
    Put Wall   — strike with highest put OI.  Acts as support.
...
class GammaChartBuilder:
    ...
    def create_chart(self, data: dict, strikes: list, option_symbols: list) -> "go.Figure":
        ...
        # Current price line — blue, bold label
        fig.add_hline(y=current_price, line_color="blue", line_width=2)
        fig.add_annotation(
            text=f"<b>${current_price:.2f}</b>",
            xref="paper", yref="y",
            x=0.01, y=current_price,
            showarrow=False, xanchor="left", yanchor="bottom",
            font=dict(size=15, color="blue"),
        )

        # Max pain line — orange dashed
        if max_pain is not None:
            fig.add_hline(
                y=max_pain, line_color="orange", line_width=2, line_dash="dash",
                annotation_text=f"Max Pain ${max_pain}",
                annotation_position="top right",
                annotation_font_color="orange", annotation_font_size=14,
            )

        # Call wall — green dotted (resistance)
        if call_wall is not None:
            fig.add_hline(
                y=call_wall, line_color="green", line_width=1, line_dash="dot",
                annotation_text=f"Call Wall ${call_wall}",
                annotation_position="bottom right",
                annotation_font_color="green", annotation_font_size=14,
            )

        # Put wall — red dotted (support)
        if put_wall is not None:
            fig.add_hline(
                y=put_wall, line_color="red", line_width=1, line_dash="dot",
                annotation_text=f"Put Wall ${put_wall}",
                annotation_position="bottom left",
                annotation_font_color="red", annotation_font_size=14,
            )

        # Invisible legend-only traces for key levels (same row format as GEX bars)
...
class DeltaChartBuilder:
    ...
    def create_chart(self, data: dict, strikes: list, option_symbols: list) -> "go.Figure":
        ...
        # Invisible legend-only traces for key levels
        if max_pain is not None and current_price:
            diff = current_price - max_pain
            direction = "↑" if diff > 0 else "↓"
            fig.add_trace(go.Scatter(
