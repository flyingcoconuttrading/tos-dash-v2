...
def calculate_walls(data: dict, strikes: list, option_symbols: list) -> tuple:
    """
    Call Wall  — strike with highest gamma-weighted OI (call gamma * call OI).
    Put Wall   — strike with highest gamma-weighted OI (put gamma  * put  OI).
    Gamma-weighting matches dealer hedging pressure (Unusual Whales methodology).
    Returns (call_wall, put_wall), either may be None.
    """
    if not strikes or not option_symbols:
        return None, None

    max_call_goi, max_put_goi = -1.0, -1.0
    ...
    for strike in strikes:
        try:
            ...
            c_goi = safe(f"{call_sym}:GAMMA") * safe(f"{call_sym}:OPEN_INT")
            p_goi = safe(f"{put_sym}:GAMMA")  * safe(f"{put_sym}:OPEN_INT")

            if c_goi > max_call_goi:
                max_call_goi = c_goi
                call_wall = strike

            if p_goi > max_put_goi:
                max_put_goi = p_goi
                put_wall = strike
