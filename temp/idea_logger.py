...
class IdeaLogger:
    ...
    def process_tick(self, candidates, data, spy_price, spy_vol_rate, spy_vol_ratio, ms, surge_syms):
        ...
        # 4. Alerts — deduplicated: same zone suppressed for alert_cooldown_s seconds
        if ms and getattr(ms, "alert_zone", "normal") != "normal":
            import time as _time
            now_ts   = _time.monotonic()
            new_zone = ms.alert_zone
            elapsed  = now_ts - self._last_alert_time
            if new_zone != self._last_alert_zone or elapsed >= self._alert_cooldown_s:
                self._log_alert(ms)
    ...
    def _update_lifecycle(self, key, state, candidate, score, mark,
        ...
        # Level cross
        if call_wall and put_wall and state.option_type == "Call" and spy_price < put_wall:
            self._invalidate(key, state, INVALIDATION_LEVEL_CROSS, spy_price,
                             mark or state.entry_mark, spy_vol_ratio, now,
                             detail=f"SPY={spy_price:.2f} crossed put_wall={put_wall:.0f}")
            return
        if call_wall and put_wall and state.option_type == "Put" and spy_price > call_wall:
            self._invalidate(key, state, INVALIDATION_LEVEL_CROSS, spy_price,
                             mark or state.entry_mark, spy_vol_ratio, now,
                             detail=f"SPY={spy_price:.2f} crossed call_wall={call_wall:.0f}")
            return
    ...
    def _log_event(self, idea_id, event_type, score=None, mark=None,
        ...
        with self._connect() as conn:
            conn.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?)
            """, [row[k] for k in ["idea_id","event_time","event_type","score","mark","spy","spy_move","vol_ratio","detail"]])
